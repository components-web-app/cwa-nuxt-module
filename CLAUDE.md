...
- **Date and time** ([#320](https://github.com/components-web-app/cwa-nuxt-module/issues/320)): `CwaUiCalendar` and `CwaUiInputDate` are ports of Nuxt UI's Calendar and InputDate; `CwaUiDatePicker` combines them and is the one to use in the admin. Its value is always a UTC ISO string or `null`; the editor picks on their computer's clock (`toLocalDateTime`/`fromLocalDateTime` in `resources/date-time-input.ts`), the offset is shown inside the field, with the zone named on hover, both **at the chosen date**, and nothing before `min` (rounded up to the minute step) is ever emitted. The dynamic single/range variants are typed as `Component`, because their props differ and Vue cannot check a union at one binding.
