...
- **Orphaned resources** ([api-components-bundle#190](https://github.com/components-web-app/api-components-bundle/issues/190)): `/_cwa/orphaned` (`_cwa-orphaned`) and a site-settings section, reached from settings and **not** the header menu. Both use one scan implementation, `useOrphanedResourceReport` (`$cwa.orphanedResources`). **A 404 from `GET /_/orphaned_resources` means no scan yet, never an error.** The scan can be async, so after the POST the report is re-read until `generatedAt` changes: once immediately, then 5 more at 2s (~10s), then "requested, reload later" — polling longer only hammers an uncached endpoint while a worker is down. Trap: `generatedAt` has one-second precision, so two scans in the same second look unchanged. Deletes go to `POST /_/orphaned_resources/delete` ([#358](https://github.com/components-web-app/cwa-nuxt-module/issues/358); needs an api-components-bundle release containing #353): Delete → `{ iris: [iri] }`, Delete all → that section's IRIs, Delete everything → `{ all: true }`, one request each after one confirmation. The server re-scans and deletes only what is still orphaned, cascades, and refreshes the stored report — so afterwards the report is **re-read**, not edited locally, and only if that re-read fails (or 404s) are the `deleted` IRIs and `not_found` rejections removed locally. `deleteOrphans` never sends `{}`, `{ all: false }` or an empty `iris` (it returns an empty result instead), and a body naming both sends only the IRIs — `all: true` must come from Delete everything alone. IRIs are sent as listed; `?published=true` is only for View. The settings notice is advisory: a failed report load shows nothing there.
