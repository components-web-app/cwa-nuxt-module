...
**[#256](https://github.com/components-web-app/cwa-nuxt-module/issues/256) — Bug (regression): cached content blanks/reloads on navigation to a previously-loaded page**
Returning to a loaded page, or switching to a new nested sub-page (shared parent), briefly blanks already-correct cached content then reloads it — should update in place, never disappear. **Regression from our recent work** — bisect suspects: **#250 manifest migration (`98f08cbb`, prime)**, redirect flash fix (`b66f7a3b`), #251 (`c4c30a34`). Machinery: `startFetch` → `resetCurrentResources([])` clears `currentIds`; the manifest batch re-fetches cached resources, re-marking them `IN_PROGRESS` (`data` retained). Render layer is *designed* to hold the old page (`displayFetchStatus` → `resolvedSuccessFetchStatus`, loaders gate on `!data`), so the blank is a timing window — likely `pageIriAtDepth(0)` briefly `undefined` → `KeepAlive :key="pageIri"` remounts. Next: bisect (start by reverting/inspecting #250) + live-watch `displayFetchStatus`/`pageIriAtDepth(0)`/`currentIds`/`apiState` during a nested nav.
