...
**[#252](https://github.com/components-web-app/cwa-nuxt-module/issues/252) — DX: multiple uploadable file fields + rename Image APIs to File/Uploadable**
Multiple file fields are a faff (admin `useCwaResourceUpload` needs 5×N aliases) or structurally unsupported (display: two `withImage()` plugins collide because `useCwaComponent` merges fixed keys via `Object.assign`). Proposal: rename `withImage`/`useCwaImage*` → `withFile`/`useCwaFile*` (or `withUploadable`; they handle any uploadable file, not just images); display — per-field `useCwaFileField(props, { fileProp })`, plus multiple `withFile()` plugins surfaced under a single `files` map on the `useCwaComponent` return keyed by `fileProp` (requires `useCwaComponent`'s plugin merge to accumulate/deep-merge the `files` key instead of shallow `Object.assign`); admin — a `bind` object on `useCwaResourceUpload` + a `<CwaResourceFileField>` wrapper. Fully-auto `<CwaResourceFileFields>` is gated on an API change (unpopulated `#[UploadableField]`s are absent from `_metadata.mediaObjects`, so empty fields aren't discoverable client-side).
