...
Let a component **type** be opt-in only: hidden from the add dialog and rejected unless a group's `allowedComponents` explicitly lists its type IRI. Per-type, metadata-driven — declared as an `#[ApiResource(explicitAllowOnly: true)]` option, flowing into component **type metadata**. The API already enforces the rule server-side but per-instance (`AbstractComponent::isPositionRestricted()` + `ComponentPositionValidator`); that method **is superseded and should be removed** (base + all subclass overrides) in favour of `explicitAllowOnly`. Work: (1) API — add the `#[ApiResource]` option, point the validator at it, expose it in metadata, remove `isPositionRestricted()`; (2) module — add `explicitAllowOnly` to `ApiDocumentationComponentMetadata` and have `AddComponentDialog.findAvailableComponents` exclude such types from groups that don't list them. Cloning (#157) must respect it too. See `## allowedComponents format contract`.
