<template>
  <div class="cwa-flex cwa-border-b cwa-border-b-stone-700 cwa-py-6 cwa-space-x-4 cwa-items-center">
    ...
    <div>
      <CwaUiFormButton v-if="getAssociatedIri(data)" :to="linkFn(getAssociatedIri(data), '_cwa-pages', '#routes')">
        <IconPages v-if="data.page" class="cwa-h-6" />
        <IconData v-else class="cwa-h-6" />
        <span class="cwa-sr-only">View</span>
      </CwaUiFormButton>
      <CwaUiFormButton v-else-if="!data.redirect" color="error" @click="$emit('deleteRoute', data['@id'])">
