    ...
    <div class="cwa-flex cwa-space-x-8">
      <div class="cwa-flex-grow">
        <FilterFormWrapper label="Search" input-id="search-input">
          <input id="search-input" type="text" class="cwa-transition-colors cwa-peer cwa-bg-stone-700/80 focus:cwa-bg-stone-700 cwa-rounded-lg cwa-border-none focus:cwa-ring-0 cwa-w-full">
      ...
      <div class="cwa-min-w-[150px]">
        <FilterFormWrapper label="Sort" input-id="filter-select">
          <FilterSelect id="filter-select" v-model="sourceModel" :options="options" class="flex-grow cwa-w-full" />
        </FilterFormWrapper>
      </div>
    </div>
  </ListContainer>
