...
export default {
  ...
  methods: {
    positionMenu(clickY, clickX, element, useOffset = true) {
      ...
      const largestHeight = window.innerHeight - elementHeight - this.heightMargin
    ...
    async open(event) {
      ...
      const originalLowestMenuItem = this.menuItems[this.menuItems.length - 1]?.resource['@id']
      this.menuItems = []
      ...
      if (this.showing && this.menuItems[this.menuItems.length - 1].resource['@id'] === originalLowestMenuItem) {
        this.showing = false
        return
      }
    ...
    positionDialog({ top, left }, useOffset = true) {
      return new Promise((resolve) => {
        ...
        [this.top, this.left] = this.positionMenu(top, left, this.$el, useOffset)
        this.arrowLeft = Math.min(Math.max(left - this.left, 10), this.$el.clientWidth - 10)
        this.$el.focus()
        resolve()
      })
    },
    positionDialogListener({ top, left }) {
      this.positionDialog({ top: top, left }, false)
    },
    close() {
      this.showing = false
    },
