<template>
  <StaticPagePadding>
    <div class="flex justify-center w-full px-6">
      <div class="grow max-w-xl md:max-w-4xl">
        <CollectionSearch v-if="false" />
        <StaticSectionTitle class="leading-[0.6]! -ml-[.3em]">
          Journal.
        </StaticSectionTitle>
        <div
        ...
        >
          <JournalItem
            ...
            :key="post['@id']"
            :route="resolveResourceLink(post, 'routePath')"
            :item="post"
            itemprop="blogPost"
          />
        </div>
    ...
    <div
      v-if="false"
      class="pb-5 flex flex-col space-y-2"
    >
      <div
        v-if="collectionItems"
      ...
      >
        <article
          ...
          :class="{ 'opacity-60': !post.routePath }"
        >
          <div
            v-if="!post.routePath"
